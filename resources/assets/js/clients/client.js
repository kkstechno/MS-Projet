    ...
    $(document).on('submit', '#addNewDepartment', function (event) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.success) {
                    displaySuccessMessage(result.message);
                    var option = "<option value=''>Choisir Département</option>";
            ...
            success: function (result) {
                if (result.success) {
                    ...
                    if(client.email && client.user_id != null){
                        $('#delete-warning').append('(Si vous enregistrez cet enregistrement sans e-mail, tous les enregistrements de ce client seront supprimés)');
                    }
