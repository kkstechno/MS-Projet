...
let tbl = $('#projects_table').DataTable({
    ...
    columns: [
        ...
        {
            data: function (row) {
                return '<a title="Editer" class="btn action-btn btn-primary btn-sm edit-btn mr-1" data-id="' +
                    row.id + '">' +
                    '<i class="cui-pencil action-icon"></i>' + '</a>' +
                    '<a title="Supprimer" class="btn action-btn btn-danger btn-sm delete-btn" data-id="' +
                    row.id + '">' +
...
$(document).on('submit', '#editFormProject', function (event) {
    ...
    let empty = $description.text().trim().replace(/ \r\n\t/g, '') === '';
    if ($('#editPrice').val() == 0) {
        displayErrorMessage('Le montant du budget doit être au minimum de 1.');
        return false;
    }
    if (removeCommas($('#editPrice').val()).length > 12) {
        displayErrorMessage('Le montant du budget peut être composé de 12 chiffres au maximum.');
        return false;
    }
    let form = $(this);
    ...
    if ($('#editDescription').summernote('isEmpty')) {
        ...
        displayErrorMessage('Le champ de description ne contient pas d\'espace blanc');
        loadingButton.button('reset');
        return false;
    }
    let formdata = $(this).serializeArray();
...
$(document).on('submit', '#addNewClientForm', function (event) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.success) {
                displaySuccessMessage(result.message);
                var option = "<option value=''>Choisir partenaire</option>";
...
$(document).on('click', '.delete-btn', function (event) {
    ...
    if (projectId === stopwatchProjectId && isClockRunning === 'true') {
        ...
        swal({
            'title': 'Avertissement',
            'text': 'Veuillez arrêter la minuterie avant de supprimer le projet.',
            'type': 'warning',
