                        ...
                        <div class="dropdown-menu dropdown-large dropdown-menu-right">
                            <div class="row">
                                <div class="form-group mb-2 col-sm-6 d-flex justify-content-start">
                                    <a class="btn btn-primary" id="resetFilters">{{__('Réinitialiser')}}</a>
                                </div>
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-sm-6 col-md-6 col-lg-4">
                                    <label class="lbl-block"><b>{{ __('Assigné à') }}</b></label>
                                    {{ Form::select('drp_users',$assignees, Auth::id(), ['id'=>'filter_user', 'class'=>'form-control min-width-150',  'placeholder' => 'Tous']) }}
                                </div>
                                @endif
                                <div class="form-group col-sm-6 col-md-6 col-lg-4">
                                    <label class="lbl-block"><b>{{ __('Projet') }}</b></label>
                                    {{Form::select('drp_project',$projects,null,['id'=>'project_filter', 'class'=>'form-control min-width-150', 'placeholder' => 'Choisir Projet'])  }}
                                </div>
                                <div class="form-group col-sm-12 col-md-6 col-lg-4 col-12">
                                    <label class="lbl-block"><b>{{ __('Date d\'échéance') }}</b></label>
                                    {{ Form::text('due_date_filter', null, ['id'=>'dueDateFilter', 'class' => 'form-control min-width-150', 'placeholder' => 'Entrer date', 'autocomplete'=>'off']) }}
                                </div>
                                <div class="form-group col-sm-12 col-md-6 col-lg-4 col-12">
                                    <label class="lbl-block"><b>{{ __('Statut') }}</b></label>
                                    {{Form::select('drp_status',$status,0,['id'=>'filter_status', 'class'=>'form-control min-width-150', 'placeholder' => 'Tous'])  }}
                                </div>
                                <div class="form-group col-sm-12 col-md-6 col-lg-4 col-12">
                                    <label class="lbl-block"><b>{{ __('Taille de la page') }}</b></label>
                                    {{ Form::select('per_page', $perPageOption, '10', ['id'=>'filter_per_page', 'class'=>'form-control perPage tasks-w-150'])  }}
                                </div>
                                    <div class="form-group col-sm-12 col-md-6 col-lg-4 col-12">
                                        <label class="lbl-block"><b>{{ __('Trier par') }}</b></label>
                                        {{ Form::select('tasks_filter', $tasksFilterOptions, \App\Models\Task::CREATED_AT_DESC, ['id'=>'filter_task', 'class'=>'form-control tasksFilter tasks-w-150 mr-3', 'placeholder' => 'Tous'])  }}
                                    </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="dropdown d-inline ">
                        ...
                        <div class="dropdown-menu copy-today-activity">
                            <a class="dropdown-item has-icon addTasksModal text-content-wrap" href="#"
                               data-placement="bottom" title="{{ __('Nouvelle tâche') }}"
                               data-delay='{"show":"500", "hide":"50"}'>
                                <i class="fas fa-plus"></i>{{ __('Ajouter tâche') }}</a>
                            <a class="dropdown-item has-icon timeEntryAddModal manuallyTaskTimeEntry text-content-wrap"
                               href="#"
                               data-placement="bottom" title="{{ __('Ajouter timing') }}"
                               data-delay='{"show":"500", "hide":"50"}'>
                                <i class="fas fa-clock"></i>{{ __('Nouvelle période') }}</a>
                            <a class="dropdown-item has-icon text-content-wrap" href="#" id="copyTodayEntry"
                               data-placement="bottom" title="{{ __('Copier activité du jour') }}"
                               data-delay='{"show":"500", "hide":"50"}'>
                                <i class="far fa-copy"></i>{{ __('Copier activité') }}</a>
                        </div>
                    </div>
                </div>
            </div>
