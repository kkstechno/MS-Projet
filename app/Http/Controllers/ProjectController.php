...
class ProjectController extends AppBaseController
{
    ...
    public function store(CreateProjectRequest $request)
    {
        ...
        return $this->sendSuccess('Projet crée avec succès.');
    }
    ...
    public function edit(Project $project)
    {
        ...
        return $this->sendResponse(['project' => $project, 'users' => $users, 'allUsers' => $allUsers], 'Projet récupéré avec succès.');
    }
    ...
    public function update(Project $project, UpdateProjectRequest $request)
    {
        ...
        if ($input['status'] == Project::STATUS_FINISHED) {
            if ($project->tasks()->where('status', '=', Task::$status['STATUS_ACTIVE'])->count() > 0) {
                return $this->sendError('Ce projet a des tâches en attente.');
            }
        }

        $this->projectRepository->update($input, $project->id);

        return $this->sendSuccess('Le projet a été mis à jour avec succès.');
    }
    ...
    public function destroy(Project $project)
    {
        ...
        return $this->sendSuccess('Projet supprimé avec succès.');
    }
    ...
    public function getMyProjects()
    {
        ...
        return $this->sendResponse($projects, 'Projet récupéré avec succès.');
    }
    ...
    public function users(Request $request)
    {
        ...
        return $this->sendResponse($users, 'Membres Récupérés avec succès.');
    }
    ...
    public function getProjectsByUser(User $user)
    {
        ...
        return $this->sendResponse($projectList, 'Projets récupérés avec succès.');
    }
    ...
    public function getLoginUsersProjects(): JsonResponse
    {
        ...
        return $this->sendResponse($projects, 'Projets récupérés avec succès.');
    }
    ...
    public function addAttachment(Project $project, Request $request)
    {
        ...
        if (isset($project->media) && $project->media->count() >= 25) {
            ...
            return $this->sendError('Vous ne pouvez pas télécharger plus de 25 fichiers', 422);
        }
        ...
        return $this->sendResponse($result, 'Le fichier a été téléchargé avec succès.');
    ...
    public function getAttachment(Project $project)
    {
        ...
        return $this->sendResponse($result, 'Pièce jointe récupérée avec succès.');
    }
...
//            $projects = getLoggedInUser()->projects()->get()->pluck('id')->toArray();
            ...
            if (empty($task)) {
                \Flash::error('Seems, vous n\'êtes pas autorisé à accéder à cet enregistrement.');
    ...
    public function deleteAttachment(Media $media, Request $request)
    {
        if (!empty($request->project_id) && !getLoggedInUser()->can('manage_projects')) {
            ...
            if (empty($task)) {
                return $this->sendError('Seems, vous n\'êtes pas autorisé à accéder à cet enregistrement.');
            }
        }

        $media->delete();

        return $this->sendSuccess('Le fichier a été supprimé avec succès.');
    }
}
