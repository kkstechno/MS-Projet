...
class UserController extends AppBaseController
{
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            if ($user->id == getLoggedInUserId()) {
                return $this->sendError('L\'utilisateur connecté ne peut pas être supprimé');
            }
            ...
            return $this->sendSuccess('Utilisateur supprimé avec succès.');
        } catch (\Throwable $e) {
            DB::rollBack();
    ...
    public function resendEmailVerification(User $user): JsonResponse
    {
        ...
        return $this->sendSuccess('L\'e-mail de vérification a été envoyé avec succès.');
    }
    ...
    public function profileUpdate(UpdateUserProfileRequest $request): JsonResponse
    {
        ...
        return $this->sendSuccess('Mise à jour du profil réussie.');
    }
    ...
    public function changePassword(ChangePasswordRequest $request): JsonResponse
    {
        ...
        if (! Hash::check($input['password_current'], $user->password)) {
            return $this->sendError('Le mot de passe actuel n\'est pas valide.');
        }

        $input['password'] = Hash::make($input['password']);
        $user->update($input);

        return $this->sendSuccess('Mot de passe mis à jour avec succès.');
    }
    ...
    {
        if ($user->id == getLoggedInUserId()) {
            return $this->sendError('L\'utilisateur connecté ne peut pas se désactiver.', 404);
        }
        $this->userRepository->activeDeActiveUser($user->id);

        return $this->sendSuccess('L\'utilisateur a été mis à jour avec succès.');
    }
    ...
    public function updateLanguage(Request $request): JsonResponse
    {
        ...
        return $this->sendSuccess('Langue mise à jour avec succès.');
    }
    ...
    public function getUserLists(): JsonResponse
    {
        ...
        return $this->sendResponse($data, 'Utilisateur récupéré avec succès.');
    }
    ...
    public function restoreUser($id): JsonResponse
    {
        ...
        return $this->sendResponse($user, 'Restauration par l\'utilisateur avec succès.');
    }
